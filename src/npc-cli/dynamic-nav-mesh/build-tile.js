 ...
 * @returns {number[]}
 */
const getTrianglesInBox = (positions, indices, box) => {
...
/**
 * @typedef BuildConfigProps
 * @property {import('recast-navigation').RecastConfig} recastConfig
 * @property {[min: THREE.Vector3Tuple, max: THREE.Vector3Tuple]} navMeshBounds
 */ 

/**
 * @param {BuildConfigProps} param0 
 */
export const buildConfig = ({ recastConfig, navMeshBounds: [navMeshBoundsMin, navMeshBoundsMax] }) => {
  //
...
/**
 * @typedef TileIntermediates
 * @property {number} tileX
 * @property {number} tileY
 * @property {RecastHeightfield} [heightfield]
 * @property {RecastCompactHeightfield} [compactHeightfield]
 * @property {RecastContourSet} [contourSet]
 */

/**
 * @typedef BuildTileMeshProps
 * @property {Float32Array} positions
 * @property {Uint32Array} indices
 * @property {import('recast-navigation').RecastConfig} recastConfig
 * @property {number} tileX
 * @property {number} tileY
 * @property {THREE.Vector3Tuple} tileBoundsMin
 * @property {THREE.Vector3Tuple} tileBoundsMax
 * @property {[THREE.Vector3Tuple, THREE.Vector3Tuple]} navMeshBounds
...
/**
 * @param {BuildTileMeshProps} param0 
 * @returns {BuildTileMeshResult}
 */
export const buildTile = ({
  ...
  /* verts and tris */
  const vertices = /** @type {number[]} */ (/** @type {ArrayLike<number>} */ (positions))
  const numVertices = indices.length
  const verticesArray = new VerticesArray()
  verticesArray.copy(vertices)

  const triangles = /** @type {number[]} */ (/** @type {ArrayLike<number>} */ (indices))
  const numTriangles = indices.length / 3
  const trianglesArray = new TrianglesArray()
  trianglesArray.copy(triangles)

  
  const tileIntermediates = /** @type {TileIntermediates} */ ({ tileX, tileY })
  ...
  const failTileMesh = (error) => {
      ...
      return { success: /** @type {const} */ (false), error, tileIntermediates, buildContext }
  }
  ...
  const expandedTileBoundsMin = /** @type {THREE.Vector3Tuple} */ ([...tileBoundsMin])
  const expandedTileBoundsMax = /** @type {THREE.Vector3Tuple} */ ([...tileBoundsMax])
